...
import textlintRulePrh from "@nyanrus/textlint-rule-prh/browser"
...
import prhRaw from "./prh.yml?raw"
...
kernel.lintText("TODO: アクセシビリティー", options).then(result => {
    console.log(result)
});

kernel.fixText("TODO: アクセシビリティー", options).then(result => {
    console.log(result)
});
...
- [\`handler.deleteProperty()\`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy/Proxy/deleteProperty)`, options).then(result => {
    const d = document.createElement("textarea");
    d.textContent = result.output;
...
export function runLintFix(content:string):string {
